<?php
  ...
  $mess_query = mysqli_query($conn, "SELECT * FROM `message` WHERE `to_id`='$_SESSION[cus_id]' ");
  ...
  if (isset($_POST['update'])){
    ...
    if (!empty($status)){
      mysqli_query($conn, "UPDATE orders SET `status` ='$status' WHERE id ='$order[id]'") or die('Query failed');
...
<div class="inner_left_pannel">
  ...
  <div class="notification-list">
            <?php
                if (mysqli_num_rows($mess_query) > 0){
                    while($fetch_mess = mysqli_fetch_assoc($mess_query)){
                        $sender = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM customers WHERE cus_id='$fetch_mess[from_id]'"));    
                        if ($fetch_mess['to_view'] == 0){
            ?>
                    <a class="notification unread" href="CustomerNotificationPage.php?view-order=<?php echo $fetch_mess['order_id']?>">
                    <img alt="photo" src="../../img/default_avatar.jpg">
                      <div class="description">
                        <div class="name"><?php echo $sender['username'];?></div>
            ...
            ?>
                      <a class="notification" href="CustomerNotificationPage.php?view-order=<?php echo $fetch_mess['order_id']?>">
                      <img alt="photo" src="../../img/default_avatar.jpg">
                        <div class="description">
                          <div class="name"><?php echo $sender['username'];?></div>
                          <article>Date: <?php echo "date"?></article>
...
<div  class="inner_right_pannel">
  ...
  <form method="post" style="display:flex; width: 950px;" class="message-detail">
    ...
    ?>
      <a href="CustomerFeedback.php?order_id=<?php echo $order['id']?>" class="update-button">Feedback</a>
    <?php
      }else{
    ?>
